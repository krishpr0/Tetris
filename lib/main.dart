    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          // Restart button
          _buildControlButton(Icons.refresh, _restartGame, color: Colors.red),
        ],
      ),
    );
  }
...
class TetrisEngine {
  ...
  Tetromino _createRandomTetromino() {
    final random = Random();
  ...
  void _spawnNewPiece() {
    ...
    nextPiece = _createRandomTetromino();
    currentPiece?.x = gridWidth ~/ 2 - (currentPiece?.width ?? 0) ~/ 2;
    currentPiece?.y = 0;
